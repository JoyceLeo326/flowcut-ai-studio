export type Sha256Fingerprint = `sha256:${string}`;

function rotateRight({
	value,
	amount,
}: {
	value: number;
	amount: number;
}): number {
	return (value >>> amount) | (value << (32 - amount));
}

const ROUND_CONSTANTS = Object.freeze([
	0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b,
	0x59f111f1, 0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01,
	0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7,
	0xc19bf174, 0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
	0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da, 0x983e5152,
	0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
	0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc,
	0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
	0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819,
	0xd6990624, 0xf40e3585, 0x106aa070, 0x19a4c116, 0x1e376c08,
	0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f,
	0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
	0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

const INITIAL_HASH = Object.freeze([
	0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f,
	0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
]);

function toBytes(value: string | Uint8Array): Uint8Array {
	return typeof value === "string" ? new TextEncoder().encode(value) : value;
}

function paddedMessage(input: Uint8Array): Uint8Array {
	const bitLength = input.length * 8;
	const paddedLength = Math.ceil((input.length + 9) / 64) * 64;
	const output = new Uint8Array(paddedLength);
	output.set(input);
	output[input.length] = 0x80;
	const view = new DataView(output.buffer);
	const high = Math.floor(bitLength / 0x1_0000_0000);
	const low = bitLength >>> 0;
	view.setUint32(paddedLength - 8, high, false);
	view.setUint32(paddedLength - 4, low, false);
	return output;
}

export function sha256Hex(value: string | Uint8Array): string {
	const message = paddedMessage(toBytes(value));
	const hash = [...INITIAL_HASH];
	const words = new Uint32Array(64);
	const view = new DataView(message.buffer, message.byteOffset, message.byteLength);

	for (let offset = 0; offset < message.length; offset += 64) {
		for (let index = 0; index < 16; index += 1) {
			words[index] = view.getUint32(offset + index * 4, false);
		}
		for (let index = 16; index < 64; index += 1) {
			const left = words[index - 15];
			const right = words[index - 2];
			const sigma0 =
				rotateRight({ value: left, amount: 7 }) ^
				rotateRight({ value: left, amount: 18 }) ^
				(left >>> 3);
			const sigma1 =
				rotateRight({ value: right, amount: 17 }) ^
				rotateRight({ value: right, amount: 19 }) ^
				(right >>> 10);
			words[index] =
				(words[index - 16] + sigma0 + words[index - 7] + sigma1) >>> 0;
		}

		let [a, b, c, d, e, f, g, h] = hash;
		for (let index = 0; index < 64; index += 1) {
			const sum1 =
				rotateRight({ value: e, amount: 6 }) ^
				rotateRight({ value: e, amount: 11 }) ^
				rotateRight({ value: e, amount: 25 });
			const choose = (e & f) ^ (~e & g);
			const temporary1 =
				(h + sum1 + choose + ROUND_CONSTANTS[index] + words[index]) >>> 0;
			const sum0 =
				rotateRight({ value: a, amount: 2 }) ^
				rotateRight({ value: a, amount: 13 }) ^
				rotateRight({ value: a, amount: 22 });
			const majority = (a & b) ^ (a & c) ^ (b & c);
			const temporary2 = (sum0 + majority) >>> 0;

			h = g;
			g = f;
			f = e;
			e = (d + temporary1) >>> 0;
			d = c;
			c = b;
			b = a;
			a = (temporary1 + temporary2) >>> 0;
		}

		hash[0] = (hash[0] + a) >>> 0;
		hash[1] = (hash[1] + b) >>> 0;
		hash[2] = (hash[2] + c) >>> 0;
		hash[3] = (hash[3] + d) >>> 0;
		hash[4] = (hash[4] + e) >>> 0;
		hash[5] = (hash[5] + f) >>> 0;
		hash[6] = (hash[6] + g) >>> 0;
		hash[7] = (hash[7] + h) >>> 0;
	}

	return hash.map((word) => word.toString(16).padStart(8, "0")).join("");
}

export function sha256Fingerprint(
	value: string | Uint8Array,
): Sha256Fingerprint {
	return `sha256:${sha256Hex(value)}`;
}

export function stableJson(value: unknown): string {
	if (value === null || typeof value !== "object") {
		const serialized = JSON.stringify(value);
		if (serialized === undefined) {
			throw new Error("Value is not portable JSON.");
		}
		return serialized;
	}
	if (Array.isArray(value)) {
		return `[${value.map((entry) => stableJson(entry)).join(",")}]`;
	}
	return `{${Object.keys(value)
		.sort()
		.filter((key) => Reflect.get(value, key) !== undefined)
		.map(
			(key) =>
				`${JSON.stringify(key)}:${stableJson(
					Reflect.get(value, key) as unknown,
				)}`,
		)
		.join(",")}}`;
}

export function fingerprintJson(value: unknown): Sha256Fingerprint {
	return sha256Fingerprint(stableJson(value));
}
