...
export function PropertiesPanel({ embedded = false }: { embedded?: boolean }) {
	...
	if (selectedElements.length === 0) {
		return (
			<div
				className={cn(
					"bg-background flex h-full flex-col items-center justify-center overflow-hidden",
					!embedded && "panel rounded-sm border",
				)}
			>
				<EmptyView />
			</div>
	...
	return (
		<div
			className={cn(
				"bg-background flex h-full overflow-hidden",
				!embedded && "panel rounded-sm border",
			)}
		>
